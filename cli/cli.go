...
func (cli *CommandLine) printUsage() {
	...
	fmt.Println("updateblock -hash HASH -data DATA - update the block")
...
func (cli *CommandLine) updateBlock(data, hash, nodeID string) {
	chain := blockchain.ContinueBlockChain(nodeID)
	defer chain.Database.Close()

	chain.UpdateBlock(data, hash)
...
func (cli *CommandLine) Run() {
	...
	updateBlockData := updateBlockCmd.String("data", "", "The update data for the block")
	...
	if updateBlockCmd.Parsed() {
		...
		cli.updateBlock(*updateBlockData, *updateBlockHash, nodeID)
	}
